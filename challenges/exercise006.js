...
export const isValidDNA = (str) => {
	...
	const DNA = ['C', 'G', 'T', 'A'];
	return [...str].every((char) => DNA.includes(char.toUpperCase()));
};
...
export const getComplementaryDNA = (str) => {
	...
	return [...str].map((char) => DNA[char.toUpperCase()]).join('');
};
...
export const isItPrime = (n) => {
	...
	// Faster to use primes > 3 always being next to multiple of 6
...
export const areWeCovered = (staff, day) => {
	...
	const employeesAvailable = staff.filter((employee) => employee.rota.includes(day));
	return employeesAvailable.length >= 3;
};
